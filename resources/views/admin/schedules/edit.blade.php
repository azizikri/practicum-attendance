...
@section('content')
    <nav class="page-breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.schedules.index') }}">Kelas</a></li>
    ...
    <div class="row">
        ...
        <div class="col-md-12 grid-margin stretch-card">
            <div class="card">
                <div class="card-body">
                    <h6 class="my-3 card-title">Edit Kelas</h6>
                    <form class="forms-sample" action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select-class-subject" id="class_subject_id" name="class_subject_id">
                                @foreach ($classSubjects as $classSubject)
                                    <option value="{{ $classSubject->id }}" @selected($classSubject->id == old('class_subject_id', $schedule->classSubject->id))>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select-pj" id="pj_id" name="pj_id">
                                @foreach ($assistants as $assistant)
                                    <option value="{{ $assistant->id }}" @selected($assistant->id == old('pj_id', $schedule->pj_id))>
                                        {{ $assistant->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select-schedule-location" id="location" name="location">
                                @foreach ($scheduleLocation::getKeys() as $location)
                                    <option value="{{ $scheduleLocation::getValue($location) }}" @selected($scheduleLocation::getValue($location) == old('location', $schedule->location))>
                                        {{ $location }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select-schedule-day" id="day" name="day">
                                @foreach ($scheduleDay::getKeys() as $day)
                                    <option value="{{ $scheduleDay::getValue($day) }}" @selected($scheduleDay::getValue($day) == old('day', $schedule->day))>
                                        {{ $day }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control select-schedule-shift" id="shift" name="shift">
                                @foreach ($scheduleShift::getKeys() as $shift)
                                    <option value="{{ $scheduleShift::getValue($shift) }}" @selected($scheduleShift::getValue($shift) == old('shift', $schedule->shift))>
                                        {{ $shift }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input name="total_session" type="text" class="form-control" id="total_session" autocomplete="off"
                                placeholder="Jumlah Pertemuan" value="{{ old('total_session', $schedule->total_session) }}">
                        </div>
